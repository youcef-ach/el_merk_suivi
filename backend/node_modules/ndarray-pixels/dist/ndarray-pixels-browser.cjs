...
let ndarray_ops = require("ndarray-ops");
ndarray_ops = __toESM(ndarray_ops, 1);
//#region src/get-pixels.browser.ts
function getPixelsInternal(buffer, mimeType) {
	...
	return createImageBitmap(blob, {
		...
		const context = new OffscreenCanvas(img.width, img.height).getContext("2d");
		context.drawImage(img, 0, 0);
		const pixels = context.getImageData(0, 0, img.width, img.height);
		return (0, ndarray.default)(new Uint8Array(pixels.data), [
			img.width,
			img.height,
			4
		], [
...
function putPixelData(array, data, frame = -1) {
	...
	if (array.shape.length === 3) if (array.shape[2] === 3) {
		ndarray_ops.default.assign((0, ndarray.default)(data, [
			array.shape[0],
			array.shape[1],
			3
		], [
		...
		ndarray_ops.default.assigns((0, ndarray.default)(data, [array.shape[0] * array.shape[1]], [4], 3), 255);
	} else if (array.shape[2] === 4) ndarray_ops.default.assign((0, ndarray.default)(data, [
		array.shape[0],
		array.shape[1],
		4
	], [
...
//#endregion
//#region src/save-pixels.browser.ts
...
async function streamCanvas(canvas, options) {
	const ab = await (await canvas.convertToBlob(options)).arrayBuffer();
