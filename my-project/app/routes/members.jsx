...
function MembersContent() {
  ...
  const fetchMembers = async () => {
    try {
      ...
      const res = await fetch('http://197.140.9.103/api/enterprises/members/list', {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch('http://197.140.9.103/api/enterprises/members/add', {
        method: 'POST',
        headers: {
  ...
  const handleRemove = async (memberId) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch(`http://197.140.9.103/api/enterprises/members/${memberId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      if (!res.ok) {
